...
def process_cue_number(raw_cue_number: list):
    ...
    return f"{list_num}/{cue_num}"


def initialize_config(user_macros, user_variables):
    ...
    with open("config/macros.xml", "r") as file:
        ...
        for macro in macro_list:
            ...
            user_macros = macros.add_user_macro(name, trigger, action, macro_uuid, arg_index,
                                                user_macros, path=macro.find("path").text)
    ...
    with open("config/variables.xml", "r") as file:
        ...
        for variable in variable_list:
            user_variables = variables.add_user_variable(variable.find("name").text,
                                                         variable.find("value").text,
                                                         user_variables, loaded_from_file=True)

    return console_network_config, osc_client, user_macros, user_variables


def run_macro(macro_uuid_to_run, osc_client, json_osc, user_macros, internal_macros,
              internal_variables, user_variables, dynamic_variables, requested_arg,
              has_eos_queries=False, mark_as_run=False, debug=False):
    """Run the given macro, and handle its run result appropriately"""
    print(f"Current mark as run: {mark_as_run}")
    run_result = ("wait", "")
    for macro in internal_macros:
        if macro.uuid == macro_uuid_to_run:
            macro.last_fire_time = datetime.datetime.now()
            run_result = macro.run_action(osc_client, internal_macros, internal_variables,
                                          user_variables, dynamic_variables, mark_as_run=True,
                                          arg_input=requested_arg, has_eos_queries=has_eos_queries,
                                          debug=debug)
    ...
    if run_result[0] == "wait":
        ...
        internal_macros = run_macro(run_result[1], osc_client, json_osc, user_macros, run_result[2],
                                    internal_variables, user_variables, dynamic_variables,
                                    requested_arg, has_eos_queries=True, mark_as_run=True,
                                    debug=debug)

        internal_macros = internal_macros
        print(f"Deleting macro {macro_uuid_to_run}")
        internal_macros = macros.delete_internal_macro(macro_uuid_to_run, internal_macros)
        print(f"Internal macros length: {len(internal_macros)}")
...
def index():
    ...
    global osc_client, macro_tree, user_macros, internal_macros, internal_variables, \
    ...
    if request.method == "GET":
        ...
        if ((request.form["submit_macro"] == "Update Macro")
                ...
                or request.form["submit_macro"] == "Run Macro"):
            ...
            if not found_macro:
                ...
                user_macros = macros.add_user_macro(new_macro_name, " ".join(
                    [new_macro_trigger_type, new_macro_trigger]),
                                                    new_macro_action,
                                                    macro_uuid_to_update,
                                                    new_macro_arg_index,
                                                    user_macros, path="")
            ...
            if request.form["submit_macro"] == "Run Macro":
                ...
                for macro in user_macros:
                    if macro.uuid == request.form["macro_uuid"]:
                        print(f"Manually running macro: {macro.uuid}")
                        fake_json = {"address": "", "args": [""]}
                        fake_requested_arg = ""
                        internal_variables = []
                        internal_macros = run_macro(macro.uuid, osc_client, fake_json, user_macros,
                                                    internal_macros, internal_variables,
                                                    user_variables, dynamic_variables,
                                                    fake_requested_arg, debug=True)
...
def network_config():
    global console_network_config, network_tree, osc_client, internal_variables, \
        user_variables, dynamic_variables
    if request.method == "POST":
        ...
        if request.form["submit"] == "Update Config":
            ...
            if osc_client:
                ...
                variables = {"internal_variables": internal_variables,
            ...
            if osc_client:
                ...
                osc_data = {"osc_addr": request.form["custom_osc_address"],
                            "osc_args": request.form["custom_osc_arguments"].split()}
                variables = {"internal_variables": internal_variables,
                             "user_variables": user_variables,
                             "dynamic_variables": dynamic_variables}
                _cleaned_osc = osc.process_osc("", osc_data, variables)
...
def handle_osc():
    ...
    global console_network_config, osc_client, user_macros, internal_macros, internal_variables, \
        user_variables, dynamic_variables
    ...
    if json_osc["address"] == "/eos/out/ping" and json_osc["args"][0] == "macroPlus":
        console_network_config[2] = f"{str(datetime.datetime.now())} - Ping validated!"
        ...
        for internal_macro in internal_macros:
            # Internal macro triggers are always OSC commands,
            # check the second word for the address.
            # Check if the address matches the given address pattern
            # and any specified arguments match the right argument pattern
            address_match = False
            args_match = True
            ...
            # If any arguments do not match the pattern,
            # or if a necessary argument doesn't exist, args do not match
            ...
            cooldown_over = ((datetime.datetime.now() -
                             internal_macro.last_fire_time).total_seconds()
                             >= macros.macro_cooldown_time)
            ...
            if address_match and args_match and cooldown_over:
                ...
                try:
                    ...
                    requested_arg = ""
                internal_macros = run_macro(internal_macro.uuid, osc_client, json_osc, user_macros,
                                            internal_macros, internal_variables, user_variables,
                                            dynamic_variables, requested_arg, debug=True)


        for user_macro in user_macros:
            # Check if user macro relies on OSC for a trigger.
            # If it does, see if it matches and run it if it does.
            if user_macro.trigger_type == "osc":
                ...
                cooldown_over = ((datetime.datetime.now() -
                                 user_macro.last_fire_time).total_seconds()
                                 >= macros.macro_cooldown_time)

                if address_match and args_match and cooldown_over:
                    # If the address matches, check that the macro's cooldown is over,
                    # then run the action.
                    try:
                        requested_arg = json_osc["args"]
                    ...
                    internal_variables = []
                    internal_macros = run_macro(user_macro.uuid, osc_client, json_osc, user_macros,
                                                internal_macros, internal_variables, user_variables,
                                                dynamic_variables, requested_arg, debug=True)

            elif user_macro.trigger_type == "cue":
                # Handle macros which fire on a given cue
                if f"/eos/out/event/cue/{user_macro.trigger_address}/fire" == json_osc["address"]:
                    # Check that the macro_cooldown_time has elapsed since the macro was last fired.
                    if ((datetime.datetime.now() - user_macro.last_fire_time).total_seconds()
                            >= macros.macro_cooldown_time):
                        ...
                        try:
                            ...
                            requested_arg = 0
                        internal_variables = []
                        internal_macros = run_macro(user_macro.uuid, osc_client, json_osc,
                                                    user_macros, internal_macros,
...
def variable_console():
    ...
    if request.method == "GET":
        ...
        return render_template("variable_console.html", user_variables=user_variables,
                               dynamic_variables=dynamic_variables, internal_variables=internal_variables)

    elif request.method == "POST":
        # Variable is being updated
        if request.form["submit_variable"] == "Add":
            # Add only if it isn't a duplicate
            user_variables = variables.add_user_variable(request.form["variable_name"],
                                                         request.form["variable_value"],
                                                         user_variables, debug=True)

        elif request.form["submit_variable"] == "Delete":
            # Delete the variable
            user_variables = variables.delete_user_variable(request.form["variable_name"],
                                                            user_variables, debug=True)

        else:
            # Set the variable to a new value
            user_variables = variables.set_user_variable(request.form["variable_name"],
                                                         request.form["variable_value"],
                                                         user_variables, debug=True)
...
console_network_config, osc_client, user_macros, user_variables = initialize_config(
    user_macros, user_variables)
